...
Screen {
	...
	property int textSize : isNxt ? 25:20
	...
	Column {
		...
		Repeater {
			...
			Item {
				height: 40
				width: parent.width
				Image {id: im1 ; anchors.left: parent.left ; anchors.leftMargin:left1 ;visible: (tileArray[index][0]).length>0; height:40; fillMode: Image.PreserveAspectFit ;source: tileArray[index][1] }
				Text {id: txt1 ; anchors.left: parent.left ; anchors.leftMargin:left2 ;visible: (tileArray[index][0]).length>0;font.pixelSize: isNxt ? 25:20; font.family: qfont.semiBold.name; text:tileArray[index][0] }
				Text {id: txt2 ; anchors.left: parent.left ; anchors.leftMargin:left3 ;visible: (tileArray[index][0]).length>0;font.pixelSize: isNxt ? 25:20; font.family: qfont.semiBold.name;  text:tileArray[index][2] }
				Text {id: txt3 ; anchors.left: parent.left ; anchors.leftMargin:left4 ;visible: (tileArray[index][0]).length>0;font.pixelSize: isNxt ? 25:20; font.family: qfont.semiBold.name;  color: (tileArray[index][3] == "down")? "red" : "green" ; text:tileArray[index][4] }
				Text {id: txt4 ; anchors.left: parent.left ; anchors.leftMargin:left5 ;visible: (tileArray[index][0]).length>0;font.pixelSize: isNxt ? 25:20; font.family: qfont.semiBold.name; color: (tileArray[index][5] == "down")? "red" : "green" ; text:tileArray[index][6]}		
			}
		}
	}
